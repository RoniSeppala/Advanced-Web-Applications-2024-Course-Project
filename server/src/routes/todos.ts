...
router.post("/deleteboard", async (req: Request, res: Response) => { //delete a board
    let todoBoards: ITodoBoard[] = [];
    ...
    try {
        const board = await TodoBoard.findOne({ _id: boardID }); //find board in database
        if (!board) {
            res.status(400).json({error: "Board not found"});
            return
        }

        await TodoBoard.deleteOne({ _id: boardID }); //delete board from database
        todoBoards = await TodoBoard.find({ usersIDs: { $in: [(req.user as any)._id] } }); //get all boards for user

        res.status(200).json({message: "Board deleted successfully", todoBoards: todoBoards}); //return all boards for user after board is deleted for updating client
        return
